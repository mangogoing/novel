...
public class MaleAssortActivity extends BaseActivity implements
		OnItemClickListener {
	...
	List<AssortDto> ClassificatList = null;
	...
	private void json2Gson(String result) {
		...
		try {
			...
			if (ok.equals("true")) {
				JSONArray arrayMale = json.getJSONArray("male");
				int lenghtMale = arrayMale.length();
				/* 不能在for循环之外new一个对象，这会造成添加到List中的为同一个对象 */
				AssortDto cOneDto = null;
				for (int i = 0; i < lenghtMale; i++) {
					cOneDto = new AssortDto();
					String name = arrayMale.getJSONObject(i).getString("name");
					String bookCount = arrayMale.getJSONObject(i).getString(
							"bookCount");
	...
	Runnable result2json = new Runnable() {
		...
		@Override
		public void run() {
//			showToast("男生分类,当前所在第" + currentPage + "页，共" + pageCount + "页");
			setListData();
		}
	};
...
//				oneList);
	...
	// if (speaker == null) {
	// speaker = Speaker.getInstance(MaleAssortActivity.this);
	// }
	// speaker.speech(text);
	// }
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		intent.setClass(MaleAssortActivity.this, BookListActivity.class);
		intent.putExtra("name", ClassificatList.get(current).getName());
